import { ConnectedAccount, useParachainApi, useSelectedAccount } from "substrate-react";
...
export default function useLiquidityPools(liquidityFilter: boolean = false): {
    ...
    useAsyncEffect(async (): Promise<void> => {
        if (!parachainApi || !assets.length) return;
        ...
        for (const pool of uniswapConstantProduct) {
            ...
            if (baseAsset && quoteAsset) {
                ...
                if (liquidityFilter) {
                    lpTokenBalance = await lpToken.balanceOf((selectedAccount as ConnectedAccount).address);
